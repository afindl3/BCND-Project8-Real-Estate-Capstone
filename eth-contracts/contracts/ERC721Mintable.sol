...
// Reference: https://github.com/hiddentao/openzeppelin-solidity
...
// TODO Create am Ownable contract
...
contract Ownable {
  ...
  function transferOwnership(address newOwner) public onlyOwner {
    // Add functionality to transfer control of the contract to a newOwner. Make sure the new owner is a real address
...
contract Pausable is Ownable {
  ...
  function setPauseState(bool state) public onlyOwner {
    require(state != _paused, "New paused state must be different than current paused state");
    if (state == true) {
      emit Paused();
    } else if (state == false){
      emit Unpaused();
...
contract ERC165 {
  ...
  */
  function _registerInterface(bytes4 interfaceId) internal {
    require(interfaceId != 0xffffffff, "Interface should no be 0xffffffff");
...
contract ERC721 is Pausable, ERC165 {
  ...
  */
  function approve(address to, uint256 tokenId) public whenNotPaused {
    ...
    require(_tokenOwner[tokenId] != to, "The approver must be different than the token owner");
    // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
    require(((msg.sender == _tokenOwner[tokenId]) || isApprovedForAll(_tokenOwner[tokenId], msg.sender)),
      "Caller must own the token or be one of the owner's approved operator's in order to add/change the token transfer approver");
  ...
  function getApproved(uint256 tokenId) public view returns (address) {
    ...
    require(_exists(tokenId), "Specified token does not exist");
  ...
  */
  function setApprovalForAll(address to, bool approved) public whenNotPaused {
    require(to != msg.sender, "Operator must be different than the caller");
  ...
  */
  ...
  function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public whenNotPaused {
    ...
    require(_checkOnERC721Received(from, to, tokenId, _data), "Unsafe transfer");
  }
  ...
  */
  ...
  function _transferFrom(address from, address to, uint256 tokenId) internal {
    ...
    require(_tokenOwner[tokenId] == from, "Address from which to transfer the token does not own the token");
...
contract ERC721Enumerable is ERC165, ERC721 {
  ...
  mapping(address => uint256[]) private _ownedTokens; // Mapping from owner to list of owned token IDs
  ...
  */
  function tokenOfOwnerByIndex(address owner, uint256 index) public view returns (uint256) {
    require(index < balanceOf(owner), "Index must be within the range of number of tokens owned");
  ...
  */
  function tokenByIndex(uint256 index) public view returns (uint256) {
    require(index < totalSupply(), "Index must be less than total number of tokens");
...
contract MyCustomERC721Token is ERC721Metadata("AF Property Token", "AF", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") {
